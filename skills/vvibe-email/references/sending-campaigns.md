...
| Recipients | `vibe_import_campaign_recipients` | recipient list + per-person merge-tag columns |
...
switch result.outcome:
  ...
  case "no_recipients":
    ask who it should go to, then pull + import the list yourself
    (vibe_list_audience_segments → references/audience-segments.md).
    if their contacts live in a spreadsheet rather than their app's database,
    say "Open the Recipients tab in the dashboard, find this campaign, and
    import your list (CSV, Google Sheet, or paste addresses). Tell me when
    you're done and I'll send."
