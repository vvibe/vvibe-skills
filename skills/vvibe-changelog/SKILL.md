...
| Commit-time reminder wired | This skill's trigger is after-the-fact, so the plugin ships a `PostToolUse` hook that fires after every user-visible `git commit`. It's present when `hooks/changelog-nudge.js` sits two directories above this skill's own directory (the `vvibe` plugin installs skill + hook together); absent when only the skill directory arrived, which is what `npx skills add` copies | Nothing will prompt you after a commit — §5's self-detection and the `references/logging.md` §3 session-close checkpoint are the only backstops, so run them. Tell the creator once, in one line: `/plugin install vvibe` (or the Codex marketplace entry) adds an automatic reminder after each commit. Then drop it — never raise it twice in a project, and never block logging on it |
